(function() {
  ...
  angular
    .module("lachesis-ldm")
    .controller("WardBlueBridgeCountController", WardBlueBridgeCountController);

  /** @ngInject */
  function WardBlueBridgeCountController(
    $rootScope,
    $scope,
    $state,
    $stateParams,
    ...
    var vm = this, wardCode = $stateParams.wardCode;
    vm.onlineCount = 0;
    vm.offlineCount = 0;
    vm.graphData = null;
    ...
    function init() {
      return deviceInfoService.getBlueBridgeConnectedGraphDetail({wardCode: wardCode}).then(function(response) {
        vm.graphData = response.map(function(wardItem) {
          wardItem.onlineCount = 0;
          wardItem.offlineCount = 0;
          if(wardItem.connectedDevices){
    ...
    function setIcons(deviceTypeCode) {
      ...
      switch(deviceTypeCode) {
        ...
        case 1004: src = link + "device_shuye.png"; break;//蓝桥
